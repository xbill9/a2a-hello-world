...
# Default Azure Container Instance (ACI) Service URL (if not overridden by command line argument)
ACI_URL = ""

async def test_a2a_aci(server_url: str):
    print(f"🧪 Validating A2A Server at Azure Container Instance: {server_url}")
    ...
    async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
        try:
            ...
            rpc_payload = {
                ...
                "params": {
                    "id": task_id,
                    "contextId": "aci-validation-context"
                },
                "id": 2
            }
...
if __name__ == "__main__":
    ...
    if not url:
        print("❌ Error: No ACI URL specified. Please provide the server URL as an argument.")
        sys.exit(1)
        
    asyncio.run(test_a2a_aci(url))
